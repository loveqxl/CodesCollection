...
public class HandVisual : MonoBehaviour
{
    ...
    public void GivePlayerACard(Cards c, int UniqueID,int handpos, bool fast = false, bool fromDeck = true)
    {
        GameObject card;
        if (fromDeck)
            card = CreateACardAtPosition(c, DeckTransform.position, new Vector3(0f, -181f, 0f));
        else
            card = CreateACardAtPosition(c, OtherCardDrawSourceTransform.position, new Vector3(0f, -181f, 0f));
        ...
        if (!fast)
        {
            ...
            if (TakeCardsOpenly)
                ...
                s.Insert(0f, card.transform.DORotate(new Vector3(0f, 181f, 0f), GlobalSettings.Instance.CardTransitionTime)); 
    ...
    public void PlayASpellFromHand(int CardID)
    {
        ...
        PlayASpellFromHand(card, CardID);
    }

    public void PlayASpellFromHand(GameObject CardVisual,int CardID)
    {
        ...
        s.OnComplete(()=>
            {
                tableVisual.PutACardOnTable(CardVisual,CardID);
            });
    }
